...
int main(int, char*[]) {
	...
	istreambuf_view expected_file_view(
	     std::istreambuf_iterator<char> { expected_file }, std::istreambuf_iterator<char> {});

	// View the desired input file as UTF-16, in Little Endian format,
	// with the source is given to us as `char`
	ztd::text::decode_view<ztd::text::basic_utf16_le<char>, istreambuf_view> input_decoding_view(input_file_view);
	// View the desired expected file as UTF-32, in Big Endian format,
	// with the source given to us as `char`
	ztd::text::decode_view<ztd::text::basic_utf32_be<char>, istreambuf_view> expected_decoding_view(
	     expected_file_view);
