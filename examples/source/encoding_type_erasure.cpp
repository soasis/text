...
int main(int, char*[]) {
	...
	ztd::text::any_encoding_of<char> encoding(ztd::text::basic_utf8<char> {});

	const char32_t source_data[] = U"\U0001F408\U0001F431\U0001F408\n\x3A\x33";
	std::string storage;
	storage.resize(std::size(source_data) * sizeof(char32_t));

	std::span<char> output(storage);
	ztd::text::any_encoding_of<char>::any_encode_state state(encoding);
