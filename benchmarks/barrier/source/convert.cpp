...
ZTD_EXTERN_C_I_ ZTD_API_LINKAGE_I_ cnc_mcerr err_pptr_psize(
...
ZTD_EXTERN_C_I_ ZTD_API_LINKAGE_I_ cnc_mcerr err_pptr_ptr(ztd_char8_t** p_output_ptr, ztd_char8_t* output_ptr_last,
...
ZTD_EXTERN_C_I_ ZTD_API_LINKAGE_I_ error_size_size size_struct_ptr_size(
	...
	if (input_ptr == nullptr || input_size < 1) {
		return error_size_size { cnc_mcerr_ok, input_size, output_size };
	}
	ztd_char32_t __c32 = *input_ptr;
	if (__c32 > __ztd_idk_detail_last_unicode_code_point) {
		return error_size_size { cnc_mcerr_invalid_sequence, 0, 0 };
	}
	if (__c32 < __first_2byte_unicode_code_point) {
		// 1 byte
		if (!_IsCounting) {
			if (output_size < 1) {
				return error_size_size { cnc_mcerr_insufficient_output, input_size, output_size };
			}
			output_ptr[0] = (ztd_char8_t)(((__c32 >> 0) & 0b01111111) | (0b00000000));
		}
		return error_size_size { cnc_mcerr_ok, 1, 1 };
	}
	else if (__c32 < __first_3byte_unicode_code_point) {
		// 2 bytes
		if (!_IsCounting) {
		...
		return error_size_size { cnc_mcerr_ok, 1, 2 };
	}
	else if (__c32 < __first_4byte_unicode_code_point) {
		// 3 bytes
		if (!_IsCounting) {
		...
		return error_size_size { cnc_mcerr_ok, 1, 3 };
	}
	else {
		// 4 bytes
		if (!_IsCounting) {
		...
		return error_size_size { cnc_mcerr_ok, 1, 4 };
	}
}
...
ZTD_EXTERN_C_I_ ZTD_API_LINKAGE_I_ error_ptr_ptr ptr_struct_ptr_size(
	...
	if (input_ptr == nullptr || input_size == 0) {
		return error_ptr_ptr { cnc_mcerr_ok, input_ptr, output_ptr };
	}
	ztd_char32_t __c32 = *input_ptr;
	if (__c32 > __ztd_idk_detail_last_unicode_code_point) {
		return error_ptr_ptr { cnc_mcerr_invalid_sequence, input_ptr, output_ptr };
	}
	++input_ptr;
	if (__c32 < __first_2byte_unicode_code_point) {
		// 1 byte
		if (!_IsCounting) {
			if (output_size < 1) {
				return error_ptr_ptr { cnc_mcerr_insufficient_output, --input_ptr, output_ptr };
			}
...
ZTD_EXTERN_C_I_ ZTD_API_LINKAGE_I_ error_size_size size_struct_ptr_ptr(ztd_char8_t* output_ptr,
	...
	if (input_ptr == nullptr || input_ptr == input_ptr_last) {
		return error_size_size { cnc_mcerr_ok, 0, (size_t)(_IsCounting ? 0 : output_ptr_last - output_ptr) };
	}
	ztd_char32_t __c32 = *input_ptr;
	++input_ptr;
