...
#define UTF_CONVERSION_BENCHMARK(FROM_N, TO_N, BIG_FROM_SUFFIX, LIL_FROM_SUFFIX, BIG_TO_SUFFIX, LIL_TO_SUFFIX)    \
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_simdutf(benchmark::State& state) {                      \
		...
		for (auto _ : state) {                                                                                  \
			...
			size_t output_written = ztd::endian::native == ztd::endian::little                                 \
			     ? simdutf::convert_utf##FROM_N##LIL_FROM_SUFFIX##_to_utf##TO_N##LIL_TO_SUFFIX(                \
			          input, input_size, output)                                                               \
			     : simdutf::convert_utf##FROM_N##BIG_FROM_SUFFIX##_to_utf##TO_N##BIG_TO_SUFFIX(                \
			          input, input_size, output);                                                              \
...
BENCHMARK(utf8_to_utf16_well_formed_simdutf);
BENCHMARK(utf8_to_utf32_well_formed_simdutf);

BENCHMARK(utf16_to_utf8_well_formed_simdutf);
BENCHMARK(utf16_to_utf32_well_formed_simdutf);
