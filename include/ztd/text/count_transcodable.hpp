...
#define ZTD_TEXT_BASIC_COUNT_TRANSCODABLE_LOOP_BODY_I_(...)                                                    \
	...
	ZTD_TEXT_BASIC_COUNT_TRANSCODABLE_LOOP_BODY_CORE_I_()
		...
		if constexpr (is_detected_v<__txt_detail::__detect_adl_text_count_transcodable_one, _WorkingInput,
		...
		else if constexpr (is_detected_v<__txt_detail::__detect_adl_internal_text_count_transcodable_one,
			...
			::ztd::ranges::span<_CodePoint, max_code_points_v<_UFromEncoding>> __intermediate(
	...
	constexpr auto count_transcodable(_Input&& __input, _FromEncoding&& __from_encoding, _ToEncoding&& __to_encoding,
		...
		_ToState& __to_state) {
		if constexpr (is_detected_v<__txt_detail::__detect_adl_text_count_transcodable, _Input, _FromEncoding,
			              _ToEncoding, _FromErrorHandler, _ToErrorHandler, _FromState, _ToState>) {
			return text_count_transcodable(text_tag<remove_cvref_t<_FromEncoding>, remove_cvref_t<_ToEncoding>> {},
		...
		else if constexpr (is_detected_v<__txt_detail::__detect_adl_internal_text_count_transcodable, _Input,
			                   _FromEncoding, _ToEncoding, _FromErrorHandler, _ToErrorHandler, _FromState,
			                   _ToState>) {
			return __text_count_transcodable(text_tag<remove_cvref_t<_FromEncoding>, remove_cvref_t<_ToEncoding>> {},
				::std::forward<_Input>(__input), ::std::forward<_FromEncoding>(__from_encoding),
				::std::forward<_ToEncoding>(__to_encoding), ::std::forward<_FromErrorHandler>(__from_error_handler),
				::std::forward<_ToErrorHandler>(__to_error_handler), __from_state, __to_state);
		}
