...
#ifndef ZTD_TEXT_COUNT_TRANSCODABLE_HPP
#define ZTD_TEXT_COUNT_TRANSCODABLE_HPP
...
namespace ztd { namespace text {
	...
	//////
	/// @addtogroup ztd_text_count_transcodable ztd::text::count_transcodable
	...
	constexpr auto basic_count_transcodable(_Input&& __input, _FromEncoding&& __from_encoding,
...
#define ZTD_TEXT_BASIC_COUNT_TRANSCODABLE_LOOP_BODY_CORE_I_()                                                    \
...
#define ZTD_TEXT_BASIC_COUNT_TRANSCODABLE_LOOP_BODY_I_(...)                                                           \
	auto __result = __VA_ARGS__(tag<_UFromEncoding, _UToEncoding> {}, ::std::move(__working_input), __from_encoding, \
	     __to_encoding, __from_error_handler, __to_error_handler, __from_state, __to_state);                         \
	ZTD_TEXT_BASIC_COUNT_TRANSCODABLE_LOOP_BODY_CORE_I_()

		if constexpr (__txt_detail::__is_detected_v<__txt_detail::__detect_adl_text_count_transcodable_one,
			              _WorkingInput, _FromEncoding, _ToEncoding, _FromErrorHandler, _ToErrorHandler, _FromState,
			              _ToState>) {
			for (;;) {
				ZTD_TEXT_BASIC_COUNT_TRANSCODABLE_LOOP_BODY_I_(text_count_transcodable_one);
			}
		}
		else if constexpr (__txt_detail::__is_detected_v<
			                   __txt_detail::__detect_adl_internal_text_count_transcodable_one, _WorkingInput,
			                   _FromEncoding, _ToEncoding, _FromErrorHandler, _ToErrorHandler, _FromState,
			                   _ToState>) {
			for (;;) {
				ZTD_TEXT_BASIC_COUNT_TRANSCODABLE_LOOP_BODY_I_(__text_count_transcodable_one);
			}
		}
		else {
			...
			::ztd::text::span<_CodePoint, max_code_points_v<_UFromEncoding>> __intermediate(__intermediate_storage);
	...
	/// to the provided @p __from_state and @p __to_state .
	///
	/// @remarks This method checks for the ADL extension point @c text_count_transcodable . It will be called if it is
	/// possible. Otherwise, this function will defer to ztd::text::basic_count_transcodable.
	//////
	template <typename _Input, typename _FromEncoding, typename _ToEncoding, typename _FromErrorHandler,
		typename _ToErrorHandler, typename _FromState, typename _ToState>
	constexpr auto count_transcodable(_Input&& __input, _FromEncoding&& __from_encoding, _ToEncoding&& __to_encoding,
		_FromErrorHandler&& __from_error_handler, _ToErrorHandler&& __to_error_handler, _FromState& __from_state,
		_ToState& __to_state) {
		if constexpr (__txt_detail::__is_detected_v<__txt_detail::__detect_adl_text_count_transcodable, _Input,
			              _FromEncoding, _ToEncoding, _FromErrorHandler, _ToErrorHandler, _FromState, _ToState>) {
			return text_count_transcodable(
		...
		else if constexpr (__txt_detail::__is_detected_v<__txt_detail::__detect_adl_internal_text_count_transcodable,
			                   _Input, _FromEncoding, _ToEncoding, _FromErrorHandler, _ToErrorHandler, _FromState,
			                   _ToState>) {
			return __text_count_transcodable(
				tag<__txt_detail::__remove_cvref_t<_FromEncoding>, __txt_detail::__remove_cvref_t<_ToEncoding>> {},
				::std::forward<_Input>(__input), ::std::forward<_FromEncoding>(__from_encoding),
				::std::forward<_ToEncoding>(__to_encoding), ::std::forward<_FromErrorHandler>(__from_error_handler),
				::std::forward<_ToErrorHandler>(__to_error_handler), __from_state, __to_state);
			...
			return basic_count_transcodable(::std::forward<_Input>(__input),
	...
	///
	/// @remarks This method will call ztd::text::count_transcodable(input, from_encoding, to_encoding,
	...
	constexpr auto count_transcodable(_Input&& __input, _FromEncoding&& __from_encoding, _ToEncoding&& __to_encoding,
		...
		auto __result
			= count_transcodable(::std::forward<_Input>(__input), ::std::forward<_FromEncoding>(__from_encoding),
			     ::std::forward<_ToEncoding>(__to_encoding), ::std::forward<_FromErrorHandler>(__from_error_handler),
			     ::std::forward<_ToErrorHandler>(__to_error_handler), __from_state, __to_state);
	...
	constexpr auto count_transcodable(_Input&& __input, _FromEncoding&& __from_encoding, _ToEncoding&& __to_encoding,
		...
		return count_transcodable(::std::forward<_Input>(__input), ::std::forward<_FromEncoding>(__from_encoding),
			::std::forward<_ToEncoding>(__to_encoding), ::std::forward<_FromErrorHandler>(__from_error_handler),
	...
	///
	/// @remarks This method will call ztd::text::count_transcodable(input, from_encoding, to_encoding,
	/// from_error_handler, to_error_handler) by creating an @c to_error_handler similar to ztd::text::default_handler.
	//////
	template <typename _Input, typename _FromEncoding, typename _ToEncoding, typename _FromErrorHandler>
	constexpr auto count_transcodable(_Input&& __input, _FromEncoding&& __from_encoding, _ToEncoding&& __to_encoding,
	...
	/// taking into account any invoked errors (like replacement from ztd::text::replacement_handler).
	///
	/// @remarks This method will call ztd::text::count_transcodable(Input, Encoding, ErrorHandler) by creating
	/// an @c error_handler similar to ztd::text::default_handler.
	//////
	template <typename _Input, typename _FromEncoding, typename _ToEncoding>
	constexpr auto count_transcodable(_Input&& __input, _FromEncoding&& __from_encoding, _ToEncoding&& __to_encoding) {
		default_handler __from_error_handler {};
		return count_transcodable(::std::forward<_Input>(__input), ::std::forward<_FromEncoding>(__from_encoding),
	...
	/// taking into account any invoked errors (like replacement from ztd::text::replacement_handler).
	///
	/// @remarks Calls ztd::text::count_transcodable(Input, Encoding) with an @c encoding that is derived from
	/// ztd::text::default_code_unit_encoding.
	//////
	template <typename _Input, typename _ToEncoding>
	constexpr auto count_transcodable(_Input&& __input, _ToEncoding&& __to_encoding) {
		using _UInput   = __txt_detail::__remove_cvref_t<_Input>;
...
#endif // ZTD_TEXT_COUNT_TRANSCODABLE_HPP
