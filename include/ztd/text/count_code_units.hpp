...
namespace ztd { namespace text {
	...
	//////
	/// @addtogroup ztd_text_count_code_units ztd::text::count_code_units
	...
	///
	/// @param[in] __input The input range (of code points) to count the code units with.
	...
	/// with a loop over the @c .encode call into an intermediate, unseen buffer being the most basic guaranteed
	...
	constexpr auto count_code_units(
		...
		if constexpr (__detail::__is_detected_v<__detail::__detect_adl_text_count_code_units, _Input, _Encoding,
			              _ErrorHandler, _State>) {
			return text_count_code_units(::std::forward<_Input>(__input), ::std::forward<_Encoding>(__encoding),
				::std::forward<_ErrorHandler>(__error_handler), __state);
		}
		else if constexpr (__detail::__is_detected_v<__detail::__detect_adl_internal_text_count_code_units, _Input,
			                   _Encoding, _ErrorHandler, _State>) {
			return __text_count_code_units(::std::forward<_Input>(__input), ::std::forward<_Encoding>(__encoding),
				::std::forward<_ErrorHandler>(__error_handler), __state);
		}
		else {
			...
			if constexpr (__detail::__is_detected_v<__detail::__detect_object_count_code_units_one, _Encoding,
				...
				::ztd::text::span<_CodeUnit, max_code_units_v<_UEncoding>> __buf_view(__code_unit_buf);

				for (;;) {
					auto __result = __detail::__basic_count_code_units_one(
						__working_input, __encoding, __error_handler, __state);
					if (__result.error_code != encoding_error::ok) {
						return _Result(::std::move(__result.input), __code_point_count, __state,
							__result.error_code, false);
					}
	...
	///
	/// @remarks Calls ztd::text::count_code_units(Input, Encoding, ErrorHandler, State) with an @c state that is
	/// created by ztd::text::make_encode_state(Encoding).
	//////
	...
	/// taking into account any invoked errors (like replacement from ztd::text::replacement_handler).
	///
	/// @remarks Calls ztd::text::count_code_units(Input, Encoding, ErrorHandler) with an @c error_handler that is
	/// similar to ztd::text::default_handler.
	//////
	template <typename _Input, typename _Encoding>
	constexpr auto count_code_units(_Input&& __input, _Encoding&& __encoding) {
		default_handler __handler {};
		return count_code_units(::std::forward<_Input>(__input), ::std::forward<_Encoding>(__encoding), __handler);
	}
	...
	/// taking into account any invoked errors (like replacement from ztd::text::replacement_handler).
	///
	/// @remarks Calls ztd::text::count_code_units(Input, Encoding) with an @c encoding that is derived from
	/// ztd::text::default_code_unit_encoding.
	//////
	template <typename _Input>
	constexpr auto count_code_units(_Input&& __input) {
		using _UInput   = __detail::__remove_cvref_t<_Input>;
		using _Encoding = default_code_unit_encoding_t<__detail::__range_value_type_t<_UInput>>;
		return count_code_units(::std::forward<_Input>(__input), _Encoding {});
	}
