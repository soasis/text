...
namespace ztd { namespace text {
	...
	namespace __detail {
		...
		inline constexpr bool __is_unicode_encoding(std::string_view __encoding_name) noexcept {
		...
		enum class __encoding_id { unknown, utf8, utf16, utf32, utf16le, utf16be, utf32le, utf32be, ascii };

		inline constexpr __encoding_id __to_encoding_id(::std::string_view __name) {
			if (__is_encoding_name_equal(__name, "UTF-8")) {
				return __encoding_id::utf8;
			}
				...
				return __encoding_id::utf32be;
			}
			else if (__is_encoding_name_equal(__name, "ASCII")) {
				return __encoding_id::ascii;
			}
			else {
				return __encoding_id::unknown;
			}
		}

		template <typename _CharType, __encoding_id _Id>
		constexpr auto __select_encoding() {
			if constexpr (_Id == __encoding_id::utf8) {
				return basic_utf8<_CharType> {};
			}
			else if constexpr (_Id == __encoding_id::utf16) {
				return basic_utf16<_CharType> {};
			}
			else if constexpr (_Id == __encoding_id::utf16le) {
				// TODO: beef up encoding_scheme to handle this better...!
				return basic_utf16_le<_CharType> {};
			}
			else if constexpr (_Id == __encoding_id::utf16be) {
				// TODO: beef up encoding_scheme to handle this better...!
				return basic_utf16_be<_CharType> {};
			}
			else if constexpr (_Id == __encoding_id::utf32) {
				return basic_utf32<_CharType> {};
			}
			else if constexpr (_Id == __encoding_id::utf32le) {
				// TODO: beef up encoding_scheme to handle this better...!
				return basic_utf32_le<_CharType> {};
			}
			else if constexpr (_Id == __encoding_id::utf32be) {
				// TODO: beef up encoding_scheme to handle this better...!
				return basic_utf32_be<_CharType> {};
			}
			else if constexpr (_Id == __encoding_id::ascii) {
				static_assert(sizeof(_CharType) == sizeof(encoding_code_unit_t<ascii>));
				return ascii {};
			}
			else {
