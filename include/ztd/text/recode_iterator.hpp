...
namespace ztd { namespace text {
	...
	public:
		...
		//////
		/// @brief Default constructs a ztd::text::recode_iterator.
		///
		/// @remarks This can only work if the underlying encodings, error handlers, and states can handle default
		/// construction.
		constexpr recode_iterator() noexcept(::std::is_nothrow_default_constructible_v<__base_from_encoding_t>
		...
		constexpr recode_iterator(range_type __range) noexcept(
			noexcept(recode_iterator(::std::move(__range), to_encoding_type {})))
		: recode_iterator(::std::move(__range), to_encoding_type {}) {
		}
		...
		constexpr recode_iterator(range_type __range, to_encoding_type __to_encoding)
		: recode_iterator(::std::move(__range), from_encoding_type {}, ::std::move(__to_encoding)) {
		}
		...
		constexpr recode_iterator(range_type __range, from_encoding_type __from_encoding,
		...
		friend constexpr bool operator==(const recode_iterator& __it, const recode_sentinel_t&) {
		...
		//////
		/// @brief Compares whether or not this iterator has truly reached the end.
		friend constexpr bool operator==(const recode_sentinel_t& __sen, const recode_iterator& __it) {
			return __it == __sen;
		}
	...
	private:
		...
		constexpr void _M_read_one() noexcept {
			...
			_Intermediate __pivot(__intermediate_storage);
			if constexpr (_IsInputOrOutput) {
				...
				auto __result    = recode_one_into_raw(__this_input_range, this->from_encoding(), __cache_view,
