...
#include <ztd/text/tests/compare_each.hpp>
...
TEST_CASE("text/encode/core", "basic usages of encode function do not explode") {
	SECTION("execution") {
		...
		ztd::text::tests::compare_each(result0, ztd::text::tests::basic_source_character_set);
		...
		if (ztd::text::contains_unicode_encoding(encoding)) {
			...
			ztd::text::tests::compare_each(result1, ztd::text::tests::unicode_sequence_truth_native_endian);
		}
	}
