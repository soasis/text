...
inline namespace ztd_text_tests_basic_runtime_any_encoding_encode {
	...
	void check_encode(BaseEncoding&& base_encoding, BasicExpected& expected0, UnicodeExpected& expected1) {
		...
		ztd::ranges::span<std::byte> result0_storage_view(result0_storage);
		auto result0 = ztd::text::encode_into(
		     input0, encoding, result0_storage_view, ztd::text::replacement_handler {}, state0);
		ztd::ranges::span<std::byte> result0_view(result0_storage_view.data(),
		...
		if (ztd::text::contains_unicode_encoding(encoding)) {
			...
			ztd::ranges::span<std::byte> result1_storage_view(result1_storage);
			auto result1 = ztd::text::encode_into(
			     input1, encoding, result1_storage_view, ztd::text::replacement_handler {}, state1);
			ztd::ranges::span<std::byte> result1_view(result1_storage_view.data(),
...
TEST_CASE("text/encode/any_encoding/encoding_scheme", "encode with byte arrays with specific endianness") {
	...
	SECTION("endian::big") {
		SECTION("ascii") {
			check_encode<ztd::endian::big>(ztd::text::ascii {},
