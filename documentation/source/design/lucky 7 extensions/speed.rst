...
- ``tag`` - The first argument to every extension point that takes a single encoding. The ``tag`` type is :doc:`ztd::text::tag\<decltype(encoding)\> </api/tag>` with any ``const``, ``volatile``, or references (``&`` and ``&&``) removed from the ``decltype`` of the ``encoding``.
- ``duo_tag`` - The first argument to every extension point that takes 2 encodings. The ``tag`` type is :doc:`ztd::text::tag\<decltype(from_encoding), decltype(to_encoding)\> </api/tag>` with any ``const``, ``volatile``, or references (``&`` and ``&&``) removed from the ``decltype`` of the two ``encoding``\ s.
